...
#open table
df <- read_xlsx("LOE_project_maps/data/FY25_LOE_Report_Data_clean_fin.xlsx",
                sheet = "Report Tables") %>%
  as_tibble() %>%
...
points_cleaned <- points_sf %>%
  select(ID, Grant, Funding, Title, Entity, Ent_Type, County, lat, long)
  
#save shapefile
st_write(points_cleaned, "LOE_project_maps/data/FY25_shapefile/LOE_FY25.shp", delete_layer = TRUE)
