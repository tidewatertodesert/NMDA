...
#simple polygons
polygons <- st_read("EDDMapS/EDDMapS_raw/polygon_raw.shp") %>%
  st_make_valid()
...
polygon_int <- polygons_sl %>%
  st_intersection(SWCD) %>%
  st_intersection(nm_COUNTY)

#write out the data
st_write(points_join, "EDDMapS/Processed_data/points_clean.shp",
         append=FALSE, delete_layer=TRUE)

length(unique(points_join$scntfcn))

#write out the data
st_write(polygon_int, "EDDMapS/Processed_data/polygon_clean.shp",
         append=FALSE, delete_layer=TRUE)

#write out the data
st_write(SWCD, "data/shapefiles/nmswcd/SWCD_updated.shp",
         append=FALSE, delete_layer=TRUE) 
